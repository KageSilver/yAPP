...
### 1. Environment
Test Plan : [TestPlan](./Resources/Jmeter/yApp%20Test.jmx)
- Tool: Apache JMeter
...
To demonstrate a successful test run, we have included a screenshot showing the results for 2 threads. However, for the other thread levels, the results were saved directly to a CSV file, and unfortunately, screenshots were not captured.
    - Screenshot: ![Screenshot](./Resources/Jmeter/screenshot.png)
  - 1 Thread:  [Report 1](./Resources/Jmeter/summary_1.csv)
  - 2 Threads: [Report 2](./Resources/Jmeter/summary_2.csv)
  - 4 Threads: [Report 4](./Resources/Jmeter/summary_4.csv)
  - 8 Threads: [Report 8](./Resources/Jmeter/summary_8.csv)
  - 16 Threads: [Report 16](./Resources/Jmeter/summary_16.csv)
...
In our proposal, we specified the need to manage a user pool of 1,000 monthly active users, including activities such as sign-up, sign-in, password changes, and account updates. While constrained by the free tier and the 128MB memory allocation, the ideal scenario would involve upgrading to a paid plan with increased resources, enabling the system to achieve the desired performance and scalability.
  ...
  1. Throughput vs. Threads
       -  ![throughput](./Resources/Jmeter/throughput.png)
  2. Latency vs. Threads
       -  ![avg](./Resources/Jmeter/Avg.png)
...
Our CI/CD pipeline is set up using Jenkins, which is hosted on a virtual machine provided by the university *(thank you rob and cs support)*.

We created two separate Jenkins pipelines:
- [Jenkinsfile for the `main` environment](../../JenkinsfileMain)
- [Jenkinsfile for the `dev` environment](../../JenkinsfileMain)

The two environments get their separate Jenkinsfiles to ensure isolated builds, which lets us test changes in one environment without triggering builds for the other.

### Continuous Integration (CI)
- Runs automated tests

    ![Running dev tests](./Resources/CICD/dev-ci-tests.png)

    ![Running main tests](./Resources/CICD/main-ci-tests.png)

- Generates build artifacts for deployment

    ![Dev artifacts](./Resources/CICD/dev-ci-artifacts.png)

    ![Main artifacts](./Resources/CICD/main-ci-artifacts.png)

### Continuous Deployment
- Deploys successful builds from the `dev` pipeline to staging

    ![Dev website updating successfully](./Resources/CICD/dev-cd-output.png)

- Deploys successful builds from the `main` pipeline to production
    ![Outdated main website](./Resources/CICD/main-cd-outdated.png)

    ![Updated main website](./Resources/CICD/main-cd-updated.png)

### Kinda Sorta Not Continuous

The group initially considered using Amazon EC2 to host our Jenkins server, since it would smoothly integrate into our cloud stack. However, we quickly realized this wouldn't be feasible due to our project exceeding the free-tier resource constraints. In order to reduce costs, we opted to use one of the university's VM instead. However, since we can only access this VM through the school wifi or VPN, we couldn't attach the git hooks successfully. Thus, we have to manually trigger the Jenkins builds after pushing changes to the repo. Although `amplify push` and `amplify publish` updates the hosted websites for us, we have to attach the resulting APK for each release.
...
![Image output from Visual Studio](./Resources/SecurityAnalysis/StaticSecurityAnalysisOutput.png)

[Here is the report](./Resources/SecurityAnalysis/InspectionReport.xml) produced by the static analysis tool when running the security analysis tool on our source code.