...
<template>
    <div>
       <div class="card-header mb-2 relative">
  
    <h3 class="text-lg font-semibold truncate">{{ props.post.postTitle }}</h3>
    <p class="text-sm text-gray-600 overflow-hidden overflow-ellipsis whitespace-nowrap">
        <strong>Created At:</strong> {{ new Date(props.post.createdAt).toLocaleString() }}
    </p>

    <!-- Three dot menu (Dropdown) -->
     
    <div class="absolute top-0 right-0" v-if="currentUser==props.post.userName" >
